...
library(ggplot2)
# Plot your datasets
cars_plot <- ggplot(cars, aes(speed, dist)) + geom_point()
cars_plot
state.x77 <- as.data.frame(state.x77)
state.x77_plot <- ggplot(state.x77, aes(Illiteracy, Income)) + geom_point()
state.x77_plot
...
cars_cor <- cor.test(cars$speed, cars$dist)
cars_cor
state_cor <- cor.test(state.x77$Illiteracy, state.x77$Income)
state_cor
```
...
cars_speed_box <- ggplot(cars, aes(speed)) + geom_boxplot()
cars_speed_box
state_illiteracy_box <- ggplot(state.x77, aes(Illiteracy)) +
  geom_boxplot()
state_illiteracy_box
```
...
theop_plot <- ggplot(Theoph, aes(Dose,conc)) + geom_point()
theop_plot
```
...
# Split state.x77
state_sample <- sample(c(TRUE,FALSE), nrow(state.x77), replace = T, prob=c(0.6,0.4))
state_train <- state.x77[state_sample, ]
state_test <- state.x77[!state_sample, ]
```
...
# Model for cars
model_cars <- lm(dist ~ speed, data = cars_train)

# Model for states
model_state <- lm(Income ~ Illiteracy, data = state_train)
```
...
# R-squared for model_cars
summary(model_cars)$r.squared

# R-squared for model_state
summary(model_state)$r.squared
```
...
# Summary for model_cars
summary(model_cars)

# Summary for model_state
summary(model_state)
```
...
cars_train$estimate <- predict(model_cars)   
cars_train$residuals <- residuals(model_cars)
...
#save predicted and residual values to df
state_train$estimate <- predict(model_state)   
state_train$residuals <- residuals(model_state)
...
```{r}
model_state_2 <- lm(Income ~ Illiteracy + `HS Grad`, data = state_train)
summary(model_state_2)
```
```
